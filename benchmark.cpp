...
int main(int argc, char **argv)
{
  ...
  for (int n : test_sizes)
  {
    ...
    // skip the first case (warmup)
    static int case_id = 0;
    bool warmup = (case_id == 0); // N=1024 as
    case_id++;

    if (!warmup)
    {
      double mflops = (2.0 * n * n) / (secs * 1e6);
      std::cout << "N=" << n
                << "  Time(s)=" << std::fixed << std::setprecision(6) << secs
                << "  MFLOP/s=" << std::setprecision(2) << mflops
                << std::endl;
    }
